...
namespace aow3.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly aow3DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = aow3Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(aow3DbContext context)
        {
            _context = context;
        }
